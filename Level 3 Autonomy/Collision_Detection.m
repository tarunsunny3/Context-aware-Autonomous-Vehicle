...
decel_lim_lcw = -200;
decel_lim_hcw = -150;
controller_gain_lcw = 90000;
controller_gain_hcw = 150000;
...
range_hr = linspace(max(0, max([average_lwc_hr - 3 * std_lwc_hr, average_hwc_hr - 3 * std_hwc_hr])), ...
                    max([average_lwc_hr + 3 * std_lwc_hr, average_hwc_hr + 3 * std_hwc_hr]), 10);
range_rr = linspace(max(0, max([average_lwc_rr - 3 * std_lwc_rr, average_hwc_rr - 3 * std_hwc_rr])), ...
                    max([average_lwc_rr + 3 * std_lwc_rr, average_hwc_rr + 3 * std_hwc_rr]), 10);
...
step_reaction_factor = 0.0001;
reaction_factor_min = 0.0001;
reaction_factor_max = 0.0001;

reaction_factor = reaction_factor_min;
reaction_times_lcw = zeros(1, num_steps);
reaction_times_hcw = zeros(1, num_steps);
collision_switch_map = containers.Map('KeyType', 'double', 'ValueType', 'any');
seq = 0;
speeds = sampleSpeeds(speed_min, speed_max, num_steps);
while reaction_factor <= reaction_factor_max
    seq = seq + 1;
    fprintf("Sequence %d is running...\n", seq);
    road_conditions_sequence = generateRoadConditions(num_steps);
    
    collision_switch_map(reaction_factor) = struct('collisions', 0,'switches', 0, 'saves', 0);
    for step=1:num_steps
        ...
        existing_data = collision_switch_map(reaction_factor);
        ...
        elseif(strcmp(result, 'Collision'))
            ...
            reaction_times_lcw(step) = 0.01 * (mean_HR_LCW/mean_RR_LCW);
            reaction_times_hcw(step) = 0.01 * (mean_HR_HCW/mean_RR_HCW);
        ...
        collision_switch_map(reaction_factor) = existing_data;
    end
    reaction_factor = reaction_factor + step_reaction_factor;
end
...
function advisory_result = Advisory_Control(speed, current_road_condition, reaction_time_factor)
...
[~, ~, ~, ~, ~, ~, ~, uD] = designControl(secureRand(), controller_gain_lcw);
